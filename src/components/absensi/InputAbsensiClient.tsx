...
export default function InputAbsensiClient({ session }: { session: any }) {
  ...
  const loadAbsensi = async () => {
    ...
    (data.data || []).forEach((r: any) => { map[Number(r.kelompok_id)] = { kelompok_id: Number(r.kelompok_id), hadir_putra: Number(r.hadir_putra), hadir_putri: Number(r.hadir_putri) }; });
    setAbsensi(map);
  };
    ...
    <div className="space-y-4">
      ...
      {Object.entries(grouped).map(([desaName, kelompokList]) => (
        <div key={desaName} className="bg-white rounded-xl shadow overflow-hidden">
          <div className="bg-green-700 text-white px-4 py-2 font-semibold text-sm">{desaName}</div>
                  ...
                  <tr key={k.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 text-center">
                      <input type="number" min="0" max={k.target_putra} value={row.hadir_putra}
                        onChange={e => setAbsensi(p => ({ ...p, [k.id]: { ...row, hadir_putra: Number(e.target.value) } }))}
                    ...
                    <td className="px-4 py-2 text-center">
                      <input type="number" min="0" max={k.target_putri} value={row.hadir_putri}
                        onChange={e => setAbsensi(p => ({ ...p, [k.id]: { ...row, hadir_putri: Number(e.target.value) } }))}
                        className="w-16 border rounded px-2 py-1 text-center text-sm" />
                    </td>
                    <td className="px-4 py-2 text-center">
