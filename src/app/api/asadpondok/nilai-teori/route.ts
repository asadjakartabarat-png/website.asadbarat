...
import { upsertPondokNilaiTeori, getPondokNilaiTeoriByPesertaPenguji } from '@/lib/turso/db';
...
export async function GET(request: NextRequest) {
  ...
  const nilai = await getPondokNilaiTeoriByPesertaPenguji(pesertaId, pengujiId);
...
export async function POST(request: NextRequest) {
  ...
  const { peserta_id, penguji_id, teori_id, nilai } = await request.json();
  if (!peserta_id || !penguji_id || !teori_id || nilai === undefined)
    return NextResponse.json({ error: 'Semua field wajib diisi' }, { status: 400 });

  if (session.role !== 'superadmin' && session.id !== penguji_id)
    return NextResponse.json({ error: 'Forbidden' }, { status: 403 });

  const result = await upsertPondokNilaiTeori({ peserta_id, penguji_id, teori_id, nilai });
