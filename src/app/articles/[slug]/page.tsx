...
import { getArticleBySlug, getRelatedArticles } from '@/lib/turso/db';
      ...
      <main className="container mx-auto px-4 py-8">
        <article className="max-w-4xl mx-auto">
          <nav className="mb-6">
            <ol className="flex items-center space-x-2 text-sm text-gray-500">
              ...
              {article.categories && (
                <>
                  <li>
                    <Link href={`/categories/${article.categories.slug}`} className="hover:text-primary-500">
          ...
          <header className="mb-8">
            {article.categories && (
              <Link href={`/categories/${article.categories.slug}`} className="inline-block text-primary-500 hover:text-primary-600 font-medium mb-4">
                {article.categories.name}
              </Link>
            )}
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center text-gray-600 space-x-4">
                {article.users && <span>Oleh {article.users.full_name}</span>}
                <span>•</span>
